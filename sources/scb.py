...
HISTORY_URL = {"start": "/Ar/HamtaNyaMynd", "end": "/Ar/HamtaNedMynd"}
...
def download_history(start_or_end):
    ...
    for year in range(2008, datetime.now().year + 1):
        ...
        r = requests.post(
            DOMAIN + HISTORY_URL[start_or_end], data={"ar": year}, headers=headers
        )
...
def extract():
    ...
    for name in ended:
        ...
        if org_nr not in org_nrs:
            ...
            current_name = [
                agency for agency in agencies if agencies[agency]["org_nr"] == org_nr
            ][0]
